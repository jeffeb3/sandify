...
export const angle = (turtle, angleDeg) => {
  turtle.angle_rad = degToRad(angleDeg) % (2.0 * Math.PI)
}

// turn right by an angle in degrees
export const right = (turtle, angleDeg) => {
  return angle(turtle, radToDeg(turtle.angle_rad + degToRad(angleDeg)))
}

// turn left by an angle in degrees
export const left = (turtle, angleDeg) => {
  return right(turtle, -angleDeg)
}

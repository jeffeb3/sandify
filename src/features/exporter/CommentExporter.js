  ...
  export() {
    ...
    switch (state.app.input) {
      case 'shape': // shapes
        ...
        this.indent()
        this.optionLines(transform, state.transform, ['spinValue', 'spinAdvanced'], state.transform.repeatEnabled && state.transform.spinEnabled)
        this.indent()
        this.optionLine(transform, state.transform, 'spinMath', state.transform.repeatEnabled && state.transform.spinEnabled && state.transform.spinAdvanced)
        this.optionLine(transform, state.transform, 'spinSwitchbacks', state.transform.repeatEnabled && state.transform.spinEnabled && !state.transform.spinAdvanced)
        this.dedent()
        this.dedent()
        ...
        this.optionLine(transform, state.transform, 'growAdvanced', state.transform.repeatEnabled && state.transform.growEnabled)
        this.indent()
        this.optionLine(transform, state.transform, 'growMath', state.transform.repeatEnabled && state.transform.growEnabled && state.transform.growAdvanced)
        this.dedent()
        this.dedent()

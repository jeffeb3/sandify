...
describe("machine reducer", () => {
  ...
  it("should handle updateMachine", () => {
    expect(
      machine({ minX: 0, maxX: 0 }, updateMachine({ minX: 50, maxX: 50 })),
    ).toEqual({
      minX: 50,
      maxX: 50,
    })
  ...
  it("should handle toggleMachineRectExpanded", () => {
    expect(
      machine({ rectangular: false }, toggleMachineRectExpanded({})),
    ).toEqual({
      rectangular: true,
      rectExpanded: true,
      polarExpanded: false,
  ...
  it("should handle toggleMachinePolarExpanded", () => {
    expect(
      machine({ rectangular: true }, toggleMachinePolarExpanded({})),
    ).toEqual({
      rectangular: false,
      rectExpanded: false,
      polarExpanded: true,
  ...
  it("should handle setMachineRectOrigin", () => {
    expect(machine({ rectOrigin: [2] }, setMachineRectOrigin([2, 3]))).toEqual({
      rectOrigin: [3],
    })
  })
})

import { coterminal, angle, onCirclePerimeter, onSegment } from '../../common/Geometry'
...
//
function clipLineCircle(start, end, size) {
  // Cases:
  ...
  let intersections = getIntersections(start, end, size)
  ...
  if (radStart > size + 1.0e-9 && radEnd > size + 1.0e-9) {
    ...
    return [
      ...
      point,
      ...traceCircle(otherPoint, end, size)
    ]
  }
...
const removeExtraPerimeterMoves = function(vertices, size) {
  let segments = []
  ...
  for (let i=0; i<vertices.length; i++) {
    ...
    if (!onCirclePerimeter(v, size) || d > dDelta) {
...
function clipAlongPerimeter(vertices, size) {
  let cleanVertices = []
  let previous = null
  ...
  for (let next=0; next<vertices.length; next++) {
    ...
    if (previous) {
      const line = clipLineCircle(previous, vertex, size)
...
// duplicate points
function cleanVertices(vertices, size) {
  let previous = null
  let cleanVertices = []
  ...
  for (let i=0; i<vertices.length; i++) {
    if (previous) {
      ...
      if (start.distance(end) > 0.001) {
        cleanVertices.push(nearestVertexCircle(vertices[i], size))
      }
    } else {
      cleanVertices.push(nearestVertexCircle(vertices[i], size))
    }
...
function optimizePerimeter(vertices, size, minimizeMoves) {
  let segments = removeExtraPerimeterMoves(vertices, size)
  ...
  for (let j=0; j<segments.length; j++) {
    ...
    if (j > 0) {
      ...
      connectedVertices.push(traceCircle(prev[prev.length-1], current[0], size))
    }
    connectedVertices.push(current)
  }
  return connectedVertices.flat()
}

function addPolarEndpoints(vertices, settings) {
  const maxRadius = settings.maxRadius

  if (settings.polarStartPoint !== 'none') {
    if (settings.polarStartPoint === 'center') {
      vertices.unshift(new Victor(0.0, 0.0))
    } else {
      const first = vertices[0]
      const scale = maxRadius / first.magnitude()
      const startPoint = first.multiply(new Victor(scale, scale))
      vertices.unshift(new Victor(startPoint.x, startPoint.y))
    }
  }

  if (settings.polarEndPoint !== 'none') {
    if (settings.polarEndPoint === 'center') {
      vertices.push(new Victor(0.0, 0.0))
    } else {
      const last = vertices[vertices.length-1]
      const scale = maxRadius / last.magnitude()
      const endPoint = last.multiply(new Victor(scale, scale))
      vertices.push(new Victor(endPoint.x, endPoint.y))
    }
  }
...
export const polishPolarVertices = function(vertices, settings) {
  vertices = addPolarEndpoints(vertices, settings)
  vertices = clipAlongPerimeter(vertices, settings.maxRadius)
  vertices = cleanVertices(vertices, settings.maxRadius)
  vertices = optimizePerimeter(vertices, settings.maxRadius, settings.minimizeMoves)
